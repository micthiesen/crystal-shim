...
|---|---|
| Design decisions, spec changes, deviations | the relevant `docs/*.md` (decisions, mechanical, sensor, electrical, controls, build) |
...
| Measured sensor/pump data (glass, capacitance, inrush, interference) | the doc that holds the provisional value, replacing or annotating it |
...
Commit the doc updates + STATE.md and push to `main` per the repo's conventions. Run `sh scripts/check.sh` for code/tool changes;
for prose-only changes run `python3 scripts/check_docs.py`. If no remote is configured,
commit locally and report that a remote remains required.

## 5. Sign Off

Print, roughly:

> State and learnings saved; report the actual commit/push result and any unrun checks.
> The next session starts from docs/STATE.md.
