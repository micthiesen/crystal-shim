...
export const controllerMountingFootprint = "CrystalShim:MountingHole_3.2mm_NPTH";
export const controllerMountingValue = "3.2mm NPTH";
...
export function identifyControllerMountsForInitialExport(board: KicadPcb) {
  const anonymous = board.footprints.filter((fp) => fp.properties.length === 0);
  if (anonymous.length !== controllerMountingHoles.length)
    throw new Error("Expected exactly four anonymous controller mounting holes");
  if (
    board.footprints.some((fp) =>
      fp.properties.some(
        (p) =>
          p.key === "Reference" &&
          controllerMountingHoles.some((hole) => hole.ref === p.value),
      ),
    )
  )
    throw new Error("Controller mounting references already exist");
  const plans = controllerMountingHoles.map((hole) => {
  ...
  for (const { fp, ref } of plans) {
    fp.libraryLink = controllerMountingFootprint;
    fp.properties = [
      new Property({
        key: "Reference",
        value: ref,
