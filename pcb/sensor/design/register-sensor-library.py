#!/usr/bin/env python3
"""Register only the fresh sensor's native library through verified Konnect.
...
LIBRARY = "CrystalShim_Sensor"
EXPECTED_PINS = {
    **{f"C{i}": {"1", "2"} for i in range(1, 5)},
    **{f"R{i}": {"1", "2"} for i in range(1, 7)},
    "D2": {"1", "2"}, "J1": {str(i) for i in range(1,7)},
    "U1": {str(i) for i in range(1,11)}, "U2": {str(i) for i in range(1,6)},
    "U3": {str(i) for i in range(1,6)}, "E1": {str(i) for i in range(1,14)},
}
...
def guarded_stage() -> Path:
    ...
    if (not value or not stage.is_absolute() or value != str(stage.resolve(strict=True))
            ...
            or not stage.name.startswith("stillair-sensor.board.main-handoff-")
            or stage.is_relative_to(HERE.parents[2])):
        raise ValueError("requires a canonical fresh sensor handoff stage outside the repository")
    if os.environ.get("STILLAIR_HANDOFF_BOARD") != BOARD_ID:
        raise ValueError("requires the complete sensor board handoff guard")
    for key, name in [
...
def validate_sensor_manifest(manifest: dict) -> None:
    ...
    if len(components) != 16 or {c["ref"] for c in components} != set(EXPECTED_PINS):
        raise ValueError("requires 16 exact sensor references")
    for component in components:
        ref = component["ref"]
        if (component["stable_id"] != f"sensor.component.{ref.lower()}"
                or component["footprint"]["kicad"] != f"{LIBRARY}:Sensor_{ref}"
                or len(component["footprint"]["pad_numbers"]) != len(EXPECTED_PINS[ref])
                or set(component["footprint"]["pad_numbers"]) != EXPECTED_PINS[ref]):
            raise ValueError(f"{ref}: exact sensor component or pin identity changed")
        if EXPECTED_PINS[ref] and component.get("symbol") != f"{LIBRARY}:Sensor_{ref}":
            raise ValueError(f"{ref}: exact sensor symbol identity changed")
...
def register() -> Path:
    ...
    validate_sensor_manifest(manifest)
    root = handoff.resolve_staged_footprint_root(stage, Path("footprints"))
    library = root / "CrystalShim_Sensor.pretty"
    expected = {f"Sensor_{item['ref']}.kicad_mod" for item in manifest["components"]}
    if len(expected) != 16 or {p.name for p in library.iterdir()} != expected:
        raise ValueError("source-exported library must contain exactly 16 sensor footprints")
    if any(not (library / name).is_file() for name in expected):
        raise ValueError("source-exported footprint is not a regular file")
    if not (stage / "sensor.kicad_pcb").is_file() or not (stage / "sensor.kicad_sch").is_file():
        raise ValueError("initial sensor board and schematic must exist")
    ...
    symbol_library = stage / "CrystalShim_Sensor.kicad_sym"
    ...
    symbol_names = {f"Sensor_{item['ref']}" for item in manifest["components"] if item['footprint']['pad_numbers']} | {"PWR_FLAG"}
    if len(symbol_names) != 17:
        raise ValueError("expected 16 source symbols and one ERC-only flag definition")
    project = stage / "sensor.kicad_pro"
    client = Konnect()
    try:
        ...
        if symbol_inventory.get("library") != str(symbol_library) or symbol_inventory.get("count") != 17 or sorted(symbol_inventory.get("symbols", [])) != sorted(symbol_names):
            raise ValueError("source-exported symbol library differs from the sensor declaration")
        ...
        result = client.call("register_footprint_library", {
            ...
            "library_path": str(library), "nickname": "CrystalShim_Sensor",
        })
        ...
        result = client.call("register_symbol_library", {
            "scope": "project", "project": str(project),
            "library_path": str(symbol_library), "nickname": "CrystalShim_Sensor",
        })
    ...
    if readback.get("count") != 1 or len(entries) != 1 or any(
        entries[0].get(key) != value for key, value in {
            "nickname": "CrystalShim_Sensor", "scope": "project",
