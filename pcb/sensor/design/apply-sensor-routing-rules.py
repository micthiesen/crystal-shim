...
def apply():
    ...
    if operation['params'].get('native_rectangle_mm') != [91, 68, 109, 132]:
    ...
    zone.SetZoneName('SENSOR_FIXED_ELECTRODES_NO_POURS')
    layers = pcbnew.LSET()
    ...
    for x, y in [(91,68),(109,68),(109,132),(91,132)]:
    ...
    if len(zones) != 1 or zones[0].GetZoneName() != 'SENSOR_FIXED_ELECTRODES_NO_POURS':
        raise ValueError('Saved rule area missing')
    z = zones[0]
    if (not z.GetIsRuleArea() or z.GetDoNotAllowTracks() or z.GetDoNotAllowVias()
    ...
    expected=sorted((pcbnew.FromMM(x),pcbnew.FromMM(y)) for x,y in [(91,68),(109,68),(109,132),(91,132)])
    ...
    with receipt.open('x') as f:
        json.dump({'schema_version':1,'operation':operation['id'],'kicad_version':pcbnew.GetBuildVersion(),
                   'rectangle_mm':[91,68,109,132],'layers':['In2.Cu','B.Cu'],
                   'fills_prohibited':True,'vias_permitted_in_declared_gap':True,'tracks_allowed_for_pad_entry':True,'source_pads_preserved':True,
