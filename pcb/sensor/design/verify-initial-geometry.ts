...
import { controllerGeometryIdentities } from "../../controller/design/footprint-geometry-identity";
...
if (
  manifest.board?.stable_id !== "sensor.board.main" ||
  manifest.components?.length !== 16 ||
  actual.size !== 16
)
  throw new Error("Initial geometry verification requires the complete sensor");
...
console.log("All 16 initial footprint geometry identities match the source manifest");
