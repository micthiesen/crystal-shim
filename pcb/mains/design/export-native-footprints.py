#!/usr/bin/env python3
"""Read an initial mains seed and native-save a fresh per-reference library.
...
registration_spec = importlib.util.spec_from_file_location("mains_registration", HERE / "register-mains-library.py")
...
def footprint_snapshot(fp: Any, pcbnew: Any) -> dict[str, Any]:
    require(fp.GetLayer() == pcbnew.F_Cu, "Only front-side mains footprints are covered")
...
def guarded_paths(args: argparse.Namespace) -> tuple[Path, Path, Path, Path]:
    ...
    board, manifest, output = stage / "mains.kicad_pcb", stage / "source-manifest.normalized.json", stage / "footprints"
    require(args.board == board and args.manifest == manifest and args.output_root == output,
            "Native export requires the exact staged mains seed, manifest and footprint root")
...
def export(args: argparse.Namespace) -> Path:
    ...
    spec = importlib.util.spec_from_file_location("mains_handoff_validation", validator_path)
    ...
    for component in components:
        ref = component["ref"]
        expected_id = f"CrystalShim_Mains:Mains_{ref}"
        ...
        library_id.SetLibNickname(pcbnew.UTF8("CrystalShim_Mains"))
    ...
    library = output / "CrystalShim_Mains.pretty"
    ...
    for ref, name, source_id, clone, expected in plans:
        ...
        entries.append({"ref": ref, "source_model": source_id,
                        ...
                        "library_id": f"CrystalShim_Mains:{name}",
