...
async function compile(rotation: number) {
  ...
  circuit.add(
    <board width={200} height={200} routingDisabled>
      <IsolatedSupply name="U1" pcbX={-28} pcbY={5} pcbRotation={rotation} schX={-8} />
      <PumpRelay name="K1" pcbX={32} pcbY={-5} pcbRotation={rotation} schX={8} />
      <netlabel net="AC_N" connectsTo=".U1 > .AC_N" />
      <netlabel net="AC_L_FUSED" connectsTo=".U1 > .AC_L" />
      <netlabel net="GND_ISO" connectsTo=".U1 > .GND_ISO" />
      <netlabel net="V5_RAW" connectsTo=".U1 > .V5_RAW" />
...
test("mains power models mirror actual bottom-view drawings and preserve contact stagger", () => {
  ...
  for (const pad of isolatedSupplyPattern.pads) {
    ...
    expect(pad.x).toBeCloseTo(45.7 - x!, 8);
    expect(pad.y).toBe(y!);
    expect(pad.drill).toBe(1.5);
    expect((pad.width - pad.drill) / 2).toBe(0.75);
  }
  ...
  test(`mains source and initial native copper retain pin geometry at ${rotation} degrees`, async () => {
    ...
    for (const [ref, pattern] of [
      ...
      const expectedNets: Record<string, string> =
        ref === "U1"
          ? { "1": "AC_N", "2": "AC_L_FUSED", "3": "GND_ISO", "4": "V5_RAW" }
          : {
              "1": "V5_PSU",
              "3": "PUMP_L_FILTERED",
              "4": "PUMP_L_SW",
...
test("mains physical declarations keep insulation separate from body and assembly bounds", () => {
  ...
  expect(supply.bodyCenter).toEqual({ x: 22.85, y: 12.7 });
  expect(supply.courtyard.width).toBeGreaterThanOrEqual(47.2);
  expect(supply.courtyard.height).toBeGreaterThanOrEqual(26.9);
  ...
  expect(supply.declaration.body.thicknessMax).toBe(22);
});
